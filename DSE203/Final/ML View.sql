...
CREATE OR REPLACE VIEW salesaggregate AS
 SELECT COALESCE(curr.nodeid, prev.nodeid) AS nodeid,
    ...
    COALESCE(rev.numreviews, 0::numeric)   AS   numreviews_by_period,
    ...
    COALESCE(( SELECT count(r.reviewid) AS numreviews
          ...
          GROUP BY (COALESCE(p.nodeid::bigint, 0::bigint))), 0::numeric) AS numreviews ,
   ...
   FROM ( SELECT s.nodeid,
           ...
           FROM sales s
          GROUP BY s.nodeid, s.month, s.monthabbr, s.year, ((s.year::text || lpad(s.month::text, 2, '0'::text))::integer), s.season, s.holidayseason, (date_part('month'::text, (date_trunc('month'::text, s.orderdate::timestamp with time zone) - '1 mon'::interval)::date)), (date_part('year'::text, (date_trunc('month'::text, s.orderdate::timestamp with time zone) - '1 mon'::interval)::date)), ((date_part('year'::text, date_trunc('month'::text, s.orderdate::timestamp with time zone) - '3 mons'::interval)::text || lpad(date_part('month'::text, date_trunc('month'::text, s.orderdate::timestamp with time zone) - '3 mons'::interval)::text, 2, '0'::text))::integer), ((date_part('year'::text, date_trunc('month'::text, s.orderdate::timestamp with time zone) - '1 year'::interval)::text || lpad(date_part('month'::text, date_trunc('month'::text, s.orderdate::timestamp with time zone) - '1 year'::interval)::text, 2, '0'::text))::integer)) curr
